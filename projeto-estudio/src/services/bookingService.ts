...
export const updateBookingStatus = async (id: string, status: 'confirmed' | 'cancelled' | 'paid' | 'pending' | 'blocked', totalPrice?: number, notes?: string) => {
  const response = await api.patch(`/bookings/${id}/status`, { status, totalPrice, notes });
  return response.data;
};
