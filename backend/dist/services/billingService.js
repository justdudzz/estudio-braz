...
export const generateInvoice = async (bookingId) => {
    try {
        ...
        const items = booking.services.map((s) => ({
 ...
 */
export const getSaftData = async (month, year, nif) => {
 ...
 */
export const generateMonthlyClosure = async (month, year) => {
    const bookings = await prisma.booking.findMany({
        where: {
    ...
    const byStaff = bookings.reduce((acc, b) => {
