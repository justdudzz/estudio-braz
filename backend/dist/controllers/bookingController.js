            ...
            for (const booking of affectedBookings) {
                if (booking.client && booking.clientId !== adminClientId) {
                    ...
                    if (booking.client.email) {
                        sendLuxuryEmail(booking.client.email, 'Aviso de Cancelamento • Studio Braz', booking.client.name, `Cancelado: ${booking.service}`, booking.date, booking.time).catch((e) => console.error("⚠️ Erro ao enviar email de cancelamento:", e));
                    }
    ...
    try {
        ...
        if (status === 'confirmed') {
            ...
            if (booking.client.email) {
                ...
                sendLuxuryEmail(booking.client.email, 'MARCAÇÃO APROVADA • Studio Braz', booking.client.name, booking.service, booking.date, booking.time).catch((e) => console.error("⚠️ Falha ao notificar cliente da aprovação:", e));
            }
        }
        else if (status === 'cancelled' || status === 'rejected') {
            if (booking.client.email) {
                sendLuxuryEmail(booking.client.email, 'MARCAÇÃO NÃO APROVADA • Studio Braz', booking.client.name, booking.service, booking.date, booking.time).catch((e) => console.error("⚠️ Falha ao notificar cliente da rejeição:", e));
            }
        }
        res.json(booking);
    }
