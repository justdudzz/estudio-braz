...
import { SERVICES_CONFIG } from '../config/businessRules.js';
// 🛠️ DICIONÁRIO DE DURAÇÕES (em minutos) — Obtido da Versão de Elite Centralizada
const SERVICE_DURATIONS = Object.fromEntries(Object.entries(SERVICES_CONFIG).map(([k, v]) => [k, v.duration]));
// 🔒 Função utilitária: Garante que existe um Client de sistema para bloqueios admin
const getOrCreateAdminClient = async () => {
...
export const createBooking = async (req, res) => {
    ...
    try {
        ...
        if (!client)
            client = await prisma.client.create({ data: { name, email: cleanEmail, phone: cleanPhone } });
        // Lógica Sequencial (Point #8 do Plano)
        if (segments && segments.length > 0) {
            let firstBookingId = null;
            const createdBookings = [];
            for (const segment of segments) {
                const services = await prisma.service.findMany({ where: { id: { in: segment.serviceIds } } });
...
export const getBusySlots = async (req, res) => {
    ...
    try {
        ...
        bookings.forEach((b) => {
            ...
            const duration = b.services.reduce((acc, s) => acc + s.service.duration, 0);
 ...
 */
export const blockSchedule = async (req, res) => {
    ...
    try {
        ...
        if (!adminStaff)
            return res.status(500).json({ message: 'Erro interno: SUPER_ADMIN não configurado' });
            ...
            for (const booking of affectedBookings) {
                if (booking.client && booking.clientId !== adminClientId) {
                    ...
                    if (booking.client.email) {
                        sendLuxuryEmail(booking.client.email, 'Aviso de Cancelamento • Studio Braz', booking.client.name, `Cancelado: serviço agendado `, booking.date, booking.time).catch((e) => console.error("⚠️ Erro ao enviar email de cancelamento:", e));
                    }
 ...
 */
export const batchDeleteBlocks = async (req, res) => {
    const { ids } = req.body;
...
export const updateBookingStatus = async (req, res) => {
    ...
    try {
        ...
        if (status === 'confirmed') {
            if (booking.client.email) {
                sendLuxuryEmail(booking.client.email, 'MARCAÇÃO APROVADA • Studio Braz', booking.client.name, 'Serviço Confirmado', booking.date, booking.time).catch((e) => console.error("⚠️ Falha ao notificar cliente da aprovação:", e));
            }
        }
        else if (status === 'paid') {
            ...
            if (booking.client.email) {
                sendLuxuryEmail(booking.client.email, 'MARCAÇÃO NÃO APROVADA • Studio Braz', booking.client.name, 'Serviço Agendado', booking.date, booking.time).catch((e) => console.error("⚠️ Falha ao notificar cliente da rejeição:", e));
            }
        }
        res.json(booking);
    }
 ...
 */
export const deepCleanDatabase = async (req, res) => {
    try {
 ...
 */
export const getStaffWithServices = async (req, res) => {
    try {
